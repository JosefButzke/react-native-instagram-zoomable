...
const styles = StyleSheet.create({
  root: {
    ...
    top:
      Platform.OS === "ios"
        ? -78
        : -(Dimensions.get("window").height / 10.078740157480315),
    left: 8.5,
    bottom: 0,
    right: 0
  },
...
export class SelectedElement extends PureComponent {
  ...
  render() {
    ...
    if (Platform.OS === "android") {
      JSON.stringify(Object.values(animatedStyle.transform[1])[0]) === "0" &&
        (animatedStyle.transform[1].translateY = 200);
    }
    ...
    return (
      <View style={styles.root}>
        {renderBackground(selected, scaleValue, gesturePosition)}
        <Animated.View style={elementStyle}>{children}</Animated.View>
      </View>
    );
  }
}
