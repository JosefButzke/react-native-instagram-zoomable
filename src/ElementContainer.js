...
export class ElementContainer extends PureComponent {
  ...
  _onGestureMove = (event: Event, gestureState: GestureState) => {
    ...
    gesturePosition.x.setValue(
      dx +
        -1 *
          parseFloat(JSON.stringify(paddingX)) *
          (scaleValue._value * scaleValue._value * 0.7)
    );

    gesturePosition.y.setValue(
      dy + parseFloat(JSON.stringify(paddingY)) * scaleValue._value
    );
